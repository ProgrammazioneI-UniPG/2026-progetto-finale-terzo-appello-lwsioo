...
void impostaGioco() {
  ...
  for (int i = 0; i < nPlayers; i++) {
    ...
    printf(GRN "Statistiche base: ATK:%d, DEF:%d, LUCK:%d\n" RESET, players[i]->attacco_psichico, players[i]->difesa_psichica, players[i]->fortuna);
...
void avanza(struct Giocatore *p) {
  ...
  void *prossima = (p->mondo == reale) ? (void *)((struct Zona_MondoReale *)p->posizione)->avanti : (void *)((struct Zona_SopraSotto *)p->posizione)->avanti;
...
void indietreggia(struct Giocatore *p) {
  ...
  void *precedente = (p->mondo == reale) ? (void *)((struct Zona_MondoReale *)p->posizione)->indietro : (void *)((struct Zona_SopraSotto *)p->posizione)->indietro;
